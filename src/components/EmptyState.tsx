  ...
  return (
    ...
    >
      <div className="relative mb-4 flex items-center justify-center">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute h-40 w-40 rounded-full bg-blue-500/20 blur-3xl dark:bg-blue-500/25"
        />
        <svg
          className="relative h-[88px] w-[88px] text-gray-800 dark:text-gray-100"
          viewBox="0 0 24 24"
          fill="currentColor"
          aria-hidden="true"
          focusable="false"
      ...
      <h2 className="bg-gradient-to-r from-blue-600 via-indigo-500 to-violet-500 bg-clip-text text-xl font-semibold text-transparent dark:from-blue-400 dark:via-indigo-300 dark:to-violet-400">
        {title}
      </h2>
      ...
      {!isGallery && (
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          {FEATURE_PILLS.map((pill) => (
            <span
              key={pill.label}
              className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white/70 px-3 py-1 text-xs text-gray-600 backdrop-blur-sm transition-transform hover:-translate-y-0.5 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-gray-300"
            >
