...
import { buildLineageGraph, buildLineageIndex, type LineageNode } from '../lib/lineage'
...
function statusColor(status: TaskRecord['status']): string {
  return status === 'done' ? 'bg-green-400' : status === 'error' ? 'bg-red-400' : 'bg-brand'
}
...
export default function LineageModal() {
  ...
  return (
    <LineagePanel
      key={lineageTaskId}
      centerId={lineageTaskId}
      close={() => setLineageTaskId(null)}
    />
  )
}
...
function LineagePanel({ centerId, close }: { centerId: string; close: () => void }) {
  ...
  const graph = useMemo(
    () => buildLineageGraph(centerId, buildLineageIndex(tasks)),
    [centerId, tasks],
  )
  ...
  const renderNode = (node: LineageNode) => {
    ...
    return (
      <button
        ...
        className={`absolute flex items-center gap-2 rounded-xl border p-1.5 text-left transition hover:bg-surface-muted dark:hover:bg-surface-raised ${
          isCenter
            ? 'border-brand ring-2 ring-brand shadow-[0_0_28px_rgba(59,130,246,0.4)] bg-brand-soft dark:bg-brand-soft'
            : 'border-line bg-white dark:border-line dark:bg-surface-muted'
        }`}
      >
        <span className="relative h-10 w-10 shrink-0 overflow-hidden rounded-md bg-surface-muted dark:bg-black/20">
          ...
          <span
            className={`absolute bottom-0.5 right-0.5 h-1.5 w-1.5 rounded-full ring-1 ring-white dark:ring-line ${statusColor(node.task.status)}`}
          />
        </span>
        <span className="min-w-0 flex-1 truncate text-xs text-content dark:text-content">
    ...
    >
      ...
      <div className="flex-1 overflow-auto p-6 custom-scrollbar">
        {graph.nodes.length <= 1 ? (
          <div className="py-20 text-center text-sm text-content-subtle dark:text-content-subtle">
            这条记录暂无可追溯的派生关系。
          </div>
        ) : (
